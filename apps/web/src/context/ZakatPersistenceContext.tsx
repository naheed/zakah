...
import { UploadedDocument, generateDocumentId, sanitizeDocumentForStorage } from '@zakatflow/core';
    ...
    useEffect(() => {
        async function loadData() {
            try {
                ...
                // 0. Intercept Deep Link (Report Mode) - Priority 1
                // This ensures we bypass "Welcome Back" logic if a report is being loaded
                const searchParams = new URLSearchParams(window.location.search);
                const deepLinkData = searchParams.get('data');

                if (deepLinkData) {
                    try {
                        console.debug('[Persistence] Deep Link detected. Attempting hydration.');
                        const jsonString = atob(deepLinkData);
                        ...
                        if (loadedData && typeof loadedData === 'object') {
                            ...
                            setReportReady(true);
                            setIsLoaded(true);
                            setHasExistingSession(false); // Disable welcome back
                            setSavedCalculationId(undefined); // Treat as new/unsaved unless we want to link it
                            return; // SKIP Local Storage loading
                        }
                    } catch (e) {
                        console.error('[Persistence] Deep link hydration failed, falling back to storage:', e);
                    }
                }
