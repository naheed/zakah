...
export function useDonationPersistence(): UseDonationPersistenceReturn {
    ...
    const addDonation = useCallback(async (
        ...
        // Optimistic Update
        const tempId = user ? crypto.randomUUID() : generateLocalId();
    ...
    const setHawlSettings = useCallback(async (date: string, calendarType: CalendarType) => {
        ...
        const newHawlSettings: HawlSettings = {
            id: hawlSettings?.id || (user ? crypto.randomUUID() : generateLocalId()),
            user_id: userId,
        ...
        const newZakatYear: ZakatYear = {
            id: currentZakatYear?.id || (user ? crypto.randomUUID() : generateLocalId()),
            user_id: userId,
            hawl_start: date,
    ...
    const setCalculatedAmount = useCallback(async (amount: number, calculationId?: string) => {
        ...
        const targetYearId = currentZakatYear?.id || (user ? crypto.randomUUID() : generateLocalId());
    ...
    const startNewYear = useCallback(async () => {
        ...
        const newYearId = user ? crypto.randomUUID() : generateLocalId();
