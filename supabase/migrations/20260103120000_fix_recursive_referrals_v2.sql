-- Fix recursive referral stats function join order
-- The previous version had a syntax error where 'rt' was used before being joined
-- Correct order: referral_tree -> referral_aggregates -> referrals
...
BEGIN
  ...
  WITH RECURSIVE referral_tree AS (
    ...
    -- Start with known people (rt), find their codes (ra), then find their referrals (child)
    SELECT 
