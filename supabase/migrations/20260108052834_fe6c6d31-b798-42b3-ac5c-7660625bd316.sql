...
create policy "Users can view their own hawl settings"
    on public.hawl_settings for select using (auth.uid() = user_id);

create policy "Users can insert their own hawl settings"
    on public.hawl_settings for insert with check (auth.uid() = user_id);

create policy "Users can update their own hawl settings"
    on public.hawl_settings for update using (auth.uid() = user_id);
...
create policy "Users can view their own zakat years"
    on public.zakat_years for select using (auth.uid() = user_id);

create policy "Users can insert their own zakat years"
    on public.zakat_years for insert with check (auth.uid() = user_id);

create policy "Users can update their own zakat years"
    on public.zakat_years for update using (auth.uid() = user_id);
...
create policy "Users can update their own donations"
    on public.donations for update using (auth.uid() = user_id);

create policy "Users can delete their own donations"
    on public.donations for delete using (auth.uid() = user_id);
