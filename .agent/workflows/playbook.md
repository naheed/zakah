...
This playbook defines how to use the 7 core ZakatFlow personas together in a fluid, continuous iteration cycle. Inspired by the orchestration patterns of **OpenClaw/Clawdbot**, we avoid rigid waterfall handoffs or "competence creep." Instead, the goal is **Continuous Refinement** via explicitly scoped, context-aware agent interactions.

## The "Orchestrator + Worker" Swarm Model

In the OpenClaw topology, agents perform best when their boundaries are strictly enforced. We mimic this by designating the `/product-manager` or `/senior-tech-lead` as the **Orchestrator** (holding the full context/memory), who then directs specialized **Worker** personas to execute focused tasks.
...
* **Role:** Specialist Workers (`/backend-ai-engineer`, `/ui-eng-designer`, `/ui-designer`, `/ux-writer`)
* **Focus:** Build and refine within strict boundaries. Prevent "competence creep."
* **Best Practice:** Give the worker *fresh* context for their specific task. The UI Engineer shouldn't be attempting to rewrite the backend SQL during this phase. 
* **Action:**
  > You: "/backend-ai-engineer Using the PM's spec, build the CSV data parser in `packages/core`. Do not touch the React UI."
